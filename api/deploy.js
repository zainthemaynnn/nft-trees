...
import { trees } from './NFT-gen/trees.js';

const PRICE_PER_NFT_IN_MONEY_UNITS = 10n;
...
export default async function deployApi(homePromise, { pathResolve }) {
  ...
  const PRICE_PER_NFT = AmountMath.make(moneyBrand, PRICE_PER_NFT_IN_MONEY_UNITS);
  const installation = await E(board).getValue(INSTALLATION_BOARD_ID);

  const issuers = harden({ Money: moneyIssuer });
  const terms = harden({
    pricePerNFT: PRICE_PER_NFT,
    nftName: "NFTree",
  });

  const {
    creatorFacet: NFTreeMarket,
    publicFacet: publicFacet,
    instance: instance,
  } = await E(zoe).startInstance(
    installation, issuers, terms
  );
  ...
  const dappConstants = {
    ...
    PRICE_PER_NFT: Number(PRICE_PER_NFT),
    API_URL,
    CONTRACT_NAME,
  };
